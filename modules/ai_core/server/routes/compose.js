...
router.post('/compose-proposal', async (req, res) => {
  try {
    ...
    const project = { name: ff.clientName || ff.projectName || 'Untitled Project', location: ff.siteAddress || '', trades: trades?.length ? trades : (ff.trades || []), inclusions: ff.inclusions || inclusions, exclusions: ff.exclusions || exclusions }
