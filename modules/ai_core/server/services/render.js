...
export async function renderTemplate(name, data) {
  const templatePath = path.join(process.cwd(), 'modules', 'ai_core', 'server', 'templates', name)
  const tpl = await fs.readFile(templatePath, 'utf8')
  const compile = Handlebars.compile(tpl)
  return compile(data)
}
export async function renderProposalHTML(data) {
  const templatePath = path.join(process.cwd(), 'modules', 'ai_core', 'server', 'templates', 'proposal.html')
  const tpl = await fs.readFile(templatePath, 'utf8')
  const compile = Handlebars.compile(tpl)
  return compile(data)
}
