...
router.post('/calc', (req, res) => {
  ...
  const items = []; for (const [empId, ag] of Object.entries(grouped)){ const emp = empIndex[empId]; if (!emp) continue; const result = calcEmployeePay({ base_rate: Number(emp.base_rate || 0), fringe_rate: Number(emp.fringe_rate || 0), davis_bacon: !!emp.davis_bacon, hours_regular: Number(ag.hours_regular || 0), hours_ot: Number(ag.hours_ot || 0), deductions: emp.deductions || {} })
    items.push(addRunItem({ run_id: run.id, employee_id: emp.id, employee_name: emp.first_name + ' ' + emp.last_name, classification: emp.classification, davis_bacon: !!emp.davis_bacon, hours_regular: ag.hours_regular, hours_ot: ag.hours_ot, gross_pay: result.gross, fringe_pay: result.fringe, taxes: result.taxes, deductions: result.deductions, net_pay: result.net })) }
...
router.post('/export', async (req, res) => {
  const { run_id } = req.body || {}; const run = getRun(run_id); if (!run) return res.status(404).json({ error: 'Run not found' }); const items = listRunItems(run.id)
  const outDir = '/mnt/data'; const wh347 = path.join(outDir, `WH347_${run.id}.pdf`); const xlsx = path.join(outDir, `PayrollSummary_${run.id}.xlsx`)
  await exportWH347(run, items, wh347); await exportPayrollSummaryXLSX(run, items, xlsx); const nacha = exportNACHA(run, items); const nacPath = path.join(outDir, `NACHA_${run.id}.txt`); await fs.writeFile(nacPath, nacha, 'utf8')
  res.json({ wh347, xlsx, nacPath })
})
