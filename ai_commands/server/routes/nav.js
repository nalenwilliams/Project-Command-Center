...
async function llmRoute(command) {
  ...
  const sys = `You map user commands to app screens. 
Output JSON: {"intent":"NAVIGATE"|"UNKNOWN","route":"/path"|"","screen_key":"", "confidence":0..1, "reason":""}`
  const r = await fetch(OPENAI_URL, {
    ...
    body: JSON.stringify({
      ...
      messages: [{role:'system',content:sys},{role:'user',content:JSON.stringify({command,registry:desc})}]
    })
  })
...
router.post('/resolve', async (req, res)=>{
  try {
    ...
    if (out.intent!=='NAVIGATE' || !out.route){
      ...
      const found = SCREEN_REGISTRY.find(s=> s.key.includes(q) || s.path.includes(q) || s.aliases.some(a=>q.includes(a)))
      if (found) out={intent:'NAVIGATE', route:found.path, screen_key:found.key, confidence:0.5, reason:'fallback'}
    }
    if (out.intent!=='NAVIGATE' || !out.route)
      return res.json({intent:'UNKNOWN', suggestions: SCREEN_REGISTRY.map(s=>s.path)})
    res.json(out)
  } catch(e){
